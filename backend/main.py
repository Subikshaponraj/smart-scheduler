...
def chat(request: ChatRequest, db: Session = Depends(get_db)):
    ...
    calendar_query_words = ['calendar', 'schedule', 'event', 'today', 'tomorrow', 'this week', 'upcoming', 'what do i have']
    is_calendar_query = any(word in user_message_lower for word in calendar_query_words)
    
    # Get events if asking about calendar
    existing_events = []
    if is_calendar_query:
        # Get events for today and upcoming days
        now = datetime.utcnow()
        today_start = now.replace(hour=0, minute=0, second=0, microsecond=0)
        upcoming_date = today_start + timedelta(days=7)  # Get next 7 days
        
        existing_events = db.query(Event).filter(
            Event.start_time >= today_start,
            Event.start_time <= upcoming_date,
            Event.status == "confirmed"
    ...
    result = extract_events_from_conversation(message_history, existing_events)
